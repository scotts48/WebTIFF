...
module NBT {
    ...
    class BitReader {
        ...
        bits: DataView = null;
        msb2lsb: boolean = true;

        byteOffset: number = 0;
        bitOffset: number = 8; //force read of first byte
        curByte: number = 0;

        constructor(bits: DataView, msb2lsb: boolean) {
            this.bits = bits;
    ...
    export class G3G4Decoder {

        public fillOrder: number = G3G4Helper.fillOrder_MSBtoLSB;
        public whiteIsZero: boolean = true;
        public imageHeight: number;
        public imageWidth: number;

        public setWhitePixels: boolean = true;
        public setBlackPixels: boolean = true;

        bitReader: BitReader = null;	// input for decoding
        pixels: ImageData = null;       // output for the pixels
        rowOffset: number;              // row offset within pixels
        ...
        refRunIndex: number = 0;
        curRunIndex: number = 0;

        vPos: number = 0;   	// the current line number we're on starting with line 0
        
        a0: number = 0;	    	// current position on (de)coding line
        // a1: number = 0;   	// next changing element to the right of a0
        // a2: number = 0;   	// next changing element to the right of a1
        b1: number = 0;		    // change element on the reference line > a0 and opposite color
        // b2: number = 0;		// next changing element on the reference line > b1
        passLength: number = 0;	// length of one or pass codes to add to the current run length
        isWhite: boolean = true;// are we currently on a white or black run
        ...
        decodeG4(bits: DataView, pixels: ImageData, rowOffset: number) {
            ...
            this.refRun = new Array<number>(this.imageWidth + 1);
            this.curRun = new Array<number>(this.imageWidth + 1);
        ...
        private getNextRefChangePos(curPos: number, curPosIsWhite: boolean) {
            ///////////////////////
            // -> REFERENCE CODE //
            ///////////////////////
            //if (curPos == 0) {
            ...
            // if the previous entry is also greater, we need to back up...
        ...
        private decodeRun(runLength: number) {
        ...
        private clearArray(array: number[], offset: number, count: number) {
        ...
        private setPixels(pixels: ImageData, x: number, y: number, count: number, setWhite: boolean) {
    ...
    interface IMakeUpCode {
        ct: number;
        crl: number;
    };

    export class G3G4Helper {

        // enable console output during decoder
        public static decoderDebugOutput: boolean = false;
        ...
        public static mode_Unknown = 0;
        public static mode_Pass = 1;
        public static mode_Horz = 2;
        public static mode_Vert0 = 3;
        public static mode_VertR1 = 4;
        public static mode_VertR2 = 5;
        public static mode_VertR3 = 6;
        public static mode_VertL1 = 7;
        public static mode_VertL2 = 8;
        public static mode_VertL3 = 9;
        public static mode_Ext2D = 10;
        public static mode_Ext1D = 11;
        public static mode_EOL = 12;

        public static code_Unknown = 0;
        public static code_Terminate = 1;
        public static code_MakeUp = 2;
        public static code_AddMakeUp = 3;
        public static code_EOL = 4;
        ...
        public static lookupMode(bitCount: number, bitValue: number): number {
        ...
        public static decodeMakeUpBlack: { [lookup: number]: IMakeUpCode } = {
            0x202: { ct: 1, crl: 3 },
            0x203: { ct: 1, crl: 2 },
            0x302: { ct: 1, crl: 1 },
            0x303: { ct: 1, crl: 4 },
        ...
        public static decodeMakeUpWhite: { [lookup: number]: IMakeUpCode } = {
            0x407: { ct: 1, crl: 2 },
            0x408: { ct: 1, crl: 3 },
            0x40b: { ct: 1, crl: 4 },
            0x40c: { ct: 1, crl: 5 },
        ...
        public static lookupDecode(isWhite: boolean, bitCount: number, bitValue: number): IMakeUpCode {
